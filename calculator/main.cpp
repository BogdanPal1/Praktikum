...
Стек из STL реализует стратегию LIFO. 
Последние добавленные элементы будут извлечены первыми. 
...
2) После этого значение будет проверено на его принадлежность числам;
...
Таким образом временная сложность будет соответствовать О(n).
